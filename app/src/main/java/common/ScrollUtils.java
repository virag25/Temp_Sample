...
public final class ScrollUtils {
	...
	/**
	 * Return a float value within the range. This is just a wrapper for
	 * Math.min() and Math.max(). This may be useful if you feel it confusing
	 * ("Which is min and which is max?").
	 * 
	 * @param value
	 *            the target value
	 * @param minValue
	 *            minimum value. If value is less than this, minValue will be
	 *            returned
	 * @param maxValue
	 *            maximum value. If value is greater than this, maxValue will be
	 *            returned
	 * @return float value limited to the range
	 */
	public static float getFloat(final float value, final float minValue,
	...
	/**
	 * Add an OnGlobalLayoutListener for the view. This is just a convenience
	 * method for using {@code ViewTreeObserver.OnGlobalLayoutListener()}. This
	 * also handles removing listener when onGlobalLayout is called.
	 * 
	 * @param view
	...
	/**
	 * Mix two colors. {@code toColor} will be {@code toAlpha/1} percent, and
	 * {@code fromColor} will be {@code (1-toAlpha)/1} percent.
	 * 
	 * @param fromColor
	 ...
	 * @param toAlpha
	 *            alpha value of toColor, 0.0f to 1.0f.
	...
	public static int mixColors(int fromColor, int toColor, float toAlpha) {
		...
		for (int i = 0; i < 4; i++) {
			result[i] = Math.min(1, fromCmyk[i] * (1 - toAlpha) + toCmyk[i]
					* toAlpha);
		}
	...
	/**
	 * Convert CYMK color to RGB color. This method doesn't check f cmyk is not
	 * null or have 4 elements in array.
	 * 
	 * @param cmyk
	 *            target CYMK color. Each value should be between 0.0f to 1.0f,
	...
	public static int rgbFromCmyk(float[] cmyk) {
		...
		int green = (int) ((1.0f - Math.min(1.0f, magenta * (1.0f - black)
				+ black)) * 255);
		int blue = (int) ((1.0f - Math.min(1.0f, yellow * (1.0f - black)
				+ black)) * 255);
